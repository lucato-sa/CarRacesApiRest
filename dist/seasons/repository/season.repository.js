...
class SeasonRepository {
    ...
    `;
        ...
        return rows.map(row => (0, database_config_1.dbToDto)('seasons', row));
    }
    async getById(id) {
        const query = `
      SELECT season_id, championship_id, descripcion, fecha_desde, fecha_hasta, pilotos_min, pilotos_max, solo_socios, rulebook_id, created_at, updated_at
    ...
    `;
        ...
        return row ? (0, database_config_1.dbToDto)('seasons', row) : undefined;
    }
    ...
    `;
        const rows = await (0, data_source_1.queryAll)(query, [championshipId]);
        return rows.map(row => (0, database_config_1.dbToDto)('seasons', row));
    }
    async create(season) {
    ...
    `;
        ...
        return row ? (0, database_config_1.dbToDto)('seasons', row) : {};
    }
