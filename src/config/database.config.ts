...
export const DATABASE_CONFIG = {
  ...
  // ============ BACKEND SELECTION ============
  BACKEND_TYPE: (process.env.BACKEND || 'memory') as 'memory' | 'file' | 'postgres' | 'supabase',
 ...
 */
export function dtoToDb(entity: keyof typeof DATABASE_CONFIG.FIELD_MAPPINGS, data: any): any {
  const mapping = DATABASE_CONFIG.FIELD_MAPPINGS[entity];
  if (!mapping) return data; // Sin mapeo, retornar datos sin cambios

  const transformed: any = {};
  Object.entries(data).forEach(([key, value]) => {
    const dbKey = mapping[key as keyof typeof mapping] || key;
 ...
 */
export function dbToDto(entity: keyof typeof DATABASE_CONFIG.FIELD_MAPPINGS, data: any): any {
  const mapping = DATABASE_CONFIG.FIELD_MAPPINGS[entity];
  if (!mapping) return data;

  const reverseMapping: Record<string, string> = {};
  Object.entries(mapping).forEach(([dtoKey, dbKey]) => {
    reverseMapping[dbKey as string] = dtoKey;
  });
