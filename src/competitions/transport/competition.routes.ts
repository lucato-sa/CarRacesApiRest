...
export function createCompetitionRoutes(repository: CompetitionRepository): Router {
  ...
  router.get('/competitions', (req: Request, res: Response) => {
    try {
  ...
  router.post('/competitions', (req: Request, res: Response) => {
    try {
      const competition = createUseCase.execute(req.body);
  ...
  router.get('/competitions/:id', (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id, 10);
      const competition = getUseCase.execute(id);
  ...
  router.put('/competitions/:id', (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id, 10);
      const competition = updateUseCase.execute(id, req.body);
      res.json(competition);
    } catch (error) {
      res.status(400).json({ error: (error as Error).message });
    }
  ...
  router.delete('/competitions/:id', (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id, 10);
